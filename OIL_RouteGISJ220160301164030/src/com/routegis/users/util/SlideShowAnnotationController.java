...
import core.routegis.engine.cache.*;
...
{
    public static final String BUFFERED_IMAGE_CACHE_SIZE = "com.routegis.avkey.BufferedImageCacheSize";
        ...
        Iterable<?> imageSources)
    {
        ...
        if (!MainClass.getMemoryCacheSet().containsCache(BUFFERED_IMAGE_CACHE_NAME))
        {
            long size = Configuration.getLongValue(BUFFERED_IMAGE_CACHE_SIZE, DEFAULT_BUFFERED_IMAGE_CACHE_SIZE);
            MemoryCache cache = new BasicMemoryCache((long) (0.85 * size), size);
            MainClass.getMemoryCacheSet().addCache(BUFFERED_IMAGE_CACHE_NAME, cache);
        }
    ...
    {
        return (PowerOfTwoPaddedImage) MainClass.getMemoryCache(BUFFERED_IMAGE_CACHE_NAME).getObject(source);
    }
    ...
    protected boolean putImage(Object source, PowerOfTwoPaddedImage image)
    {
        ...
        MemoryCache cache = MainClass.getMemoryCache(BUFFERED_IMAGE_CACHE_NAME);
    ...
    protected String getImageName(Object imageSource)
    {
        ...
        s = InOut.stripTrailingSeparator(s);
