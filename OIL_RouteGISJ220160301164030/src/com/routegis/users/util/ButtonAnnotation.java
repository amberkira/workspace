...
public class ButtonAnnotation extends ImageAnnotation implements SelectListener
{
    ...
    protected PrivateTexture pressedMaskTexture;
    // Event listeners.
    ...
    public void setPressedMaskSource(Object source)
    {
        ...
        if (source != null)
        {
            this.pressedMaskTexture = new BasicTexture(source, false);
        }
    }

    public PrivateTexture getPressedMaskTexture()
    {
    ...
    protected void applyBackgroundTextureState(DrawContext dc, int width, int height, double opacity, PrivateTexture texture)
    {
    ...
    {
        PrivateTexture texture = this.getPressedMaskTexture();
        if (texture == null)
            return;
