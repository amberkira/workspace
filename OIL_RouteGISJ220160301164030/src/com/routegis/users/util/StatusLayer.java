...
import core.routegis.engine.geom.coords.*;
import core.routegis.engine.layers.AbstractLayer;
...
{
    public final static String UNIT_METRIC = "com.rotuegis.users.util.StatusLayer.Metric";
    public final static String UNIT_IMPERIAL = "com.rotuegis.users.util.StatusLayer.Imperial";
    ...
    public StatusLayer()
    {
        ...
        Timer downloadTimer = new Timer(300, new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent actionEvent)
            {
                ...
                if (isNetworkAvailable.get() && MainClass.getRetrievalService().hasActiveTasks())
                {
        ...
        Timer netCheckTimer = new Timer(10000, new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent actionEvent)
            {
                ...
                Thread t = new Thread(new Runnable()
                {
                    public void run()
                    {
                        isNetworkAvailable.set(!MainClass.getNetworkStatus().isNetworkUnavailable());
                    }
                });
                t.start();
            }
    ...
    private void initBGTexture(DrawContext dc)
    {
        ...
        catch (IOException e)
        {
            ...
            Logging.logger().severe(msg);
            throw new PrivateRuntimeException(msg, e);
        }
    }
    ...
    protected String makeEyeAltitudeDescription(double metersAltitude)
    {
        ...
        if (UNIT_IMPERIAL.equals(elevationUnit))
            return String.format("%s %,d mi", altitude, (int) Math.round(PrivateMath.convertMetersToMiles(metersAltitude)));
    ...
    protected String makeCursorElevationDescription(double metersElevation)
    {
        ...
        if (UNIT_IMPERIAL.equals(elevationUnit))
            return String.format("%s %,d feet", elev, (int) Math.round(PrivateMath.convertMetersToFeet(metersElevation)));
        else // Default to metric units.
