...
public class SessionState
{
    ...
    {
        if (PrivateUtil.isEmpty(sessionKey))
        {
    ...
    protected void saveViewState(WorldWindow worldWindow)
    {
        ...
        try
        {
            ...
            if (PrivateUtil.isEmpty(stateInXml))
                return;
            ...
            InOut.makeParentDirs(this.getViewStatePath());
            InOut.writeTextFile(stateInXml, new File(this.getViewStatePath()));
        }
        catch (Exception e)
        {
    ...
    protected void restoreViewState(WorldWindow worldWindow)
    {
        ...
        try
        {
            ...
            String stateInXml = InOut.readTextFile(stateFile);
            if (PrivateUtil.isEmpty(stateInXml))
            {
    ...
    protected void saveLayerListState(WorldWindow worldWindow)
    {
        ...
        {
            if (stateFile.exists())
                InOut.deleteDirectory(stateFile);
            else
                stateFile.mkdirs();
    ...
    protected void saveLayerState(Layer layer, File stateFile)
    {
        try
        {
            ...
            InOut.writeTextFile(stateInXml, stateFile);
        }
        catch (Exception e)
        {
    ...
    protected String getSessionStatePath()
    {
        ...
        return InOut.appendPathPart(appDataPath, sessionDataPath);
    }

    protected String getViewStatePath()
    {
        return InOut.appendPathPart(this.getSessionStatePath(), VIEW_STATE_PATH);
    }

    protected String getLayerStatePath()
    {
        return InOut.appendPathPart(this.getSessionStatePath(), LAYER_STATE_PATH);
    }

    protected static LayerStateFilename parseLayerStateFilename(String filename)
    {
        String filenameWithoutSuffix = InOut.replaceSuffix(filename, "");
        String[] tokens = filenameWithoutSuffix.split(LAYER_STATE_FILENAME_DELIMITER);

        if (tokens == null || tokens.length < 2)
            return null;

        Integer index = PrivateUtil.makeInteger(tokens[0]);
        if (index == null)
            return null;
