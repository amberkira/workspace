...
import com.routegis.users.util.RandomShapeAttributes;
...
import core.routegis.engine.formats.geojson.*;
...
public class GeoJSONLoader
{
    ...
    {
        if (PrivateUtil.isEmpty(docSource))
        {
        ...
        catch (IOException e)
        {
            ...
            throw new PrivateRuntimeException(message, e);
        }
        finally
        {
            InOut.closeStream(doc, docSource.toString());
        }
    }
        ...
        AVList properties)
    {
        ...
        {
            p.setAltitudeMode(MainClass.ABSOLUTE);
            p.setLineEnabled(true);
        }
        else
        {
            p.setAltitudeMode(MainClass.CLAMP_TO_GROUND);
        }
