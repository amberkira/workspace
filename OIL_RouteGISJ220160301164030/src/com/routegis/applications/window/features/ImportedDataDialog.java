...
import core.routegis.engine.globes.Earth;
...
public class ImportedDataDialog extends AbstractFeatureDialog implements NetworkActivitySignal.NetworkUser
{
    ...
    public void initialize(Controller controller)
    {
        ...
        this.fileStore = MainClass.getDataFileStore();
        ...
        this.importThread = new Thread(new Runnable()
        {
            public void run()
            {
                ...
                try
                {
                    ...
                    try
                    {
                        // Import the file into a form usable by RouteGIS SDK components.
    ...
    {
        String[] names = InOut.listDescendantFilenames(dir, new DataConfigurationFilter(), false);
        ...
        for (String filename : names)
        {
            ...
            try
            {
                ...
                doc = PrivateXML.openDocument(dataConfigFile);
            ...
            // current version of RouteGIS SDK's data importers produced it. This data configuration file may have been
            // created by a previous version of RouteGIS SDK, or by another program. Set fallback values for any missing
            // parameters that RouteGIS SDK needs to construct a Layer or ElevationModel from this data configuration.
    ...
    {
        XPath xpath = PrivateXML.makeXPath();
        ...
        String s = PrivateXML.getText(domElement, "DataCacheName", xpath);
        ...
        {
            if (PrivateXML.getDouble(domElement, "ExtremeElevations/@min", xpath) == null)
                params.setValue(AVKey.ELEVATION_MIN, Earth.ELEVATION_MIN);
            if (PrivateXML.getDouble(domElement, "ExtremeElevations/@max", xpath) == null)
        ...
        DataStoreProducer producer) throws Exception
    {
        ...
        try
        {
            // Convert the file to a form usable by RouteGIS SDK components, according to the specified DataStoreProducer.
