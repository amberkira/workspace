...
import com.routegis.applications.window.features.swinglayermanager.*;
import com.routegis.applications.window.features.swinglayermanager.LayerTree;
...
import core.routegis.engine.wms.CapabilitiesRequest;
...
//            "http://wms.jpl.nasa.gov/wms.cgi"};
    ...
    public void initialize(final Controller controller)
    {
        ...
        this.urlField.addActionListener(new ActionListener() // listen for triggers to cause WMS server contact
        {
            public void actionPerformed(ActionEvent actionEvent)
            {
                try
                {
                    ...
                    if (!PrivateUtil.isEmpty(serverURLString))
                    {
        ...
        this.infoButton.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent actionEvent)
            {
                ...
                if (!PrivateUtil.isEmpty(urlString))
                {
    ...
    protected void createLayer(LayerNode layerNode)
    {
        ...
        if (layerNode.getWmsLayerInfo() != null)
        {
            ...
            // Some wms servers are slow, so increase the timeouts and limits used by RouteGIS SDK's retrievers.
        ...
        LayerTreeModel model)
    {
        ...
        if (subLayers != null && subLayers.size() > 0) // it's a grouping layer
        {
            ...
            if (!PrivateUtil.isEmpty(toolTipText))
    ...
    protected JPanel makeTopPanel()
    {
        ...
        this.infoButton = new JButton(
            ImageLibrary.getIcon("images/info-20x20.png"));
